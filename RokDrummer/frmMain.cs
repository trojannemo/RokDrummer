...
using NautilusFREE;
using XInput = Vortice.XInput.XInput;
using XState = Vortice.XInput.State;
using XGamepad = Vortice.XInput.Gamepad;
using XBtns = Vortice.XInput.GamepadButtons;
...
namespace RokDrummer
{
    public partial class frmMain : Form
    {
        ...
        // RB drums mappings (XInput / Xbox-style)
        private const XBtns FLAGS_SNARE = XBtns.RightThumb | XBtns.B;
        private const XBtns FLAGS_TOM_Y = XBtns.RightThumb | XBtns.Y;
        private const XBtns FLAGS_TOM_B = XBtns.RightThumb | XBtns.X;
        private const XBtns FLAGS_TOM_G = XBtns.RightThumb | XBtns.A;
        private const XBtns FLAGS_HIHAT_OPEN = XBtns.DPadUp | XBtns.RightShoulder | XBtns.Y;
        private const XBtns FLAGS_HIHAT_CLOSED = XBtns.DPadUp | XBtns.LeftThumb | XBtns.RightShoulder | XBtns.Y;
        private const XBtns FLAGS_RIDE = XBtns.DPadDown | XBtns.RightShoulder | XBtns.X;
        private const XBtns FLAGS_CRASH = XBtns.RightShoulder | XBtns.A;
        private const XBtns FLAGS_KICK_PEDAL = XBtns.LeftShoulder;
        private const XBtns FLAGS_HIHAT_PEDAL = XBtns.LeftThumb;

        // PS3-style mappings (your original intent preserved)
        private const XBtns FLAGS_SNARE_PS3 = XBtns.LeftThumb | XBtns.A;
        private const XBtns FLAGS_TOM_Y_PS3 = XBtns.LeftThumb | XBtns.X;
        private const XBtns FLAGS_TOM_B_PS3 = XBtns.LeftThumb | XBtns.Y;
        private const XBtns FLAGS_TOM_G_PS3 = XBtns.LeftThumb | XBtns.B;

        private const XBtns FLAGS_HIHAT_OPEN_PS3 = XBtns.DPadUp | XBtns.RightThumb | XBtns.X;

        private const XBtns FLAGS_HIHAT_CLOSED_PS3 =
            XBtns.RightShoulder | XBtns.DPadUp | XBtns.RightThumb | XBtns.X;

        private const XBtns FLAGS_RIDE_PS3 = XBtns.DPadDown | XBtns.RightThumb | XBtns.Y;
        private const XBtns FLAGS_CRASH_PS3 = XBtns.RightThumb | XBtns.B;
        private const XBtns FLAGS_HIHAT_PEDAL_PS3 = XBtns.RightShoulder;
        ...
        // Choose the bits that matter for pad decoding (no Start/Back/etc.)
        private const XBtns PAD_MASK = XBtns.A | XBtns.B | XBtns.X | XBtns.Y |
            XBtns.LeftThumb | XBtns.RightThumb |
            XBtns.LeftShoulder | XBtns.RightShoulder |
            XBtns.DPadUp | XBtns.DPadDown;
        ...
        private void ConnectionTimer_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                if (XInput.GetCapabilities(drumUserIndex, DeviceQueryType.Any, out _))
                {
                    lblConnect.Text = "Connected";
                    lblConnect.ForeColor = Color.LawnGreen;
                    DrumsTimer.Enabled = true;
                    lblDebug.Visible = debugDrumInput.Checked;
                    /*if (Drums.IsConnected && string.IsNullOrEmpty(DefaultDebuggingText) && debugDrumInput.Checked)
                    {
        ...
        {
            float rawValue = 0;
            XInput.GetState(drumUserIndex, out var state);
        ...
        // Exact combo match within a mask
        static bool Match(XBtns state, XBtns combo, XBtns mask) => (state & mask) == combo;

        // If you still want “contains” checks for menu nav:
        static bool Has(XBtns s, XBtns f) => (s & f) == f;

        private void DrumsTimer_Tick(object sender, EventArgs e)
        {
            try
            {
                bool isConnected = XInput.GetState(drumUserIndex, out XState state);
                if (!isConnected) return; // bail if controller not connected

                XGamepad gamepad = state.Gamepad;
                int velocity = CalculateDrumVelocity(gamepad);
                XBtns buttons = gamepad.Buttons;

                if (debugDrumInput.Checked)
                {
                    var debug = GetDebugData(gamepad);
                    if (debug != DefaultDebuggingText) lblDebug.Text = debug;
                }

                // D-Pad actions (use flags, not equality)
                if (Has(buttons, XBtns.DPadLeft) && !STATE_LEFT) ChangeKit(true);
                else if (Has(buttons, XBtns.DPadRight) && !STATE_RIGHT) ChangeKit(false);
                else if (Has(buttons, XBtns.DPadUp) && !STATE_UP) UpdateDrumVolume(false);
                else if (Has(buttons, XBtns.DPadDown) && !STATE_DOWN) UpdateDrumVolume(true);
                else if (Has(buttons, XBtns.Start) && !STATE_START) btnPlay.PerformClick();
                // Back button: replace this with whatever your app needs
                else if (Has(buttons, XBtns.Back))
                {
                    //ToggleSomething(); // e.g., toggle metronome, overlay, etc.
                }

                // Inputs
                CurrentInputState[Drumkit.HiHatClosed] = HitHiHatClosed(buttons)
                    || (forceClosedHihat.Checked && HitHiHatOpen(buttons));

                CurrentInputState[Drumkit.HiHatOpen] =
                    !CurrentInputState[Drumkit.HiHatClosed] && !forceClosedHihat.Checked && HitHiHatOpen(buttons);
                ...
                // Edge tracking
                STATE_LEFT = Has(buttons, XBtns.DPadLeft);
                STATE_RIGHT = Has(buttons, XBtns.DPadRight);
                STATE_UP = Has(buttons, XBtns.DPadUp);
                STATE_DOWN = Has(buttons, XBtns.DPadDown);
                STATE_START = Has(buttons, XBtns.Start);
                ...
                for (var i = 0; i < DRUM_PARTS; i++)
                {
                    ...
                    {
                        if (!STATE_INPUT[i]) PlaySample(i, velocity);
                        else { STATE_INPUT[i] = false; UpdateDrumHits(i); }
                    }
        ...
        {
            if (!XInput.GetCapabilities(stageKitIndex, DeviceQueryType.Any, out _) || stageKitDisabled.Checked) return;
            stageKit = new StageKitController(stageKitIndex + 1);
            switch (index)
            {
                case 0:
        ...
        private bool HitSnare(XBtns state)
        {
            XBtns snare = FLAGS_SNARE;
            if (pS3EKit.Checked) snare = FLAGS_SNARE_PS3;
            else if (pS3GH5Kit.Checked) snare = XBtns.A;
            else if (rockBand1.Checked) snare = XBtns.B;

            return Match(state, snare, PAD_MASK);
        }

        private bool HitTomYellow(XBtns state)
        {
            if (rockBand1.Checked) return Match(state, XBtns.Y, PAD_MASK);

            if (pS3EKit.Checked)
                return Match(state, FLAGS_TOM_Y_PS3, PAD_MASK);
            return Match(state, FLAGS_TOM_Y, PAD_MASK);
        }

        private bool HitTomBlue(XBtns state)
        {
            if (rockBand1.Checked) return Match(state, XBtns.X, PAD_MASK);
            if (pS3GH5Kit.Checked) return Match(state, XBtns.Y, PAD_MASK);

            if (pS3EKit.Checked)
                return Match(state, FLAGS_TOM_B_PS3, PAD_MASK);
            return Match(state, FLAGS_TOM_B, PAD_MASK);
        }

        private bool HitTomGreen(XBtns state)
        {
            if (rockBand1.Checked) return Match(state, XBtns.A, PAD_MASK);
            if (pS3GH5Kit.Checked) return Match(state, XBtns.B, PAD_MASK);

            if (pS3EKit.Checked)
                return Match(state, FLAGS_TOM_G_PS3, PAD_MASK);
            return Match(state, FLAGS_TOM_G, PAD_MASK);
        }

        private bool HitHiHatOpen(XBtns state)
        {
            if (rockBand1.Checked) return false;
            if (pS3GH5Kit.Checked) return Match(state, XBtns.X, PAD_MASK);

            var hihat = pS3EKit.Checked ? FLAGS_HIHAT_OPEN_PS3 : FLAGS_HIHAT_OPEN;
            return Match(state, hihat, PAD_MASK);
        }

        private bool HitHiHatClosed(XBtns state)
        {
            if (rockBand1.Checked) return false;
            var hihat = pS3EKit.Checked ? FLAGS_HIHAT_CLOSED_PS3 : FLAGS_HIHAT_CLOSED;
            return Match(state, hihat, PAD_MASK);
        }

        private bool HitRide(XBtns state)
        {
            if (rockBand1.Checked) return false;
            var ride = pS3EKit.Checked ? FLAGS_RIDE_PS3 : FLAGS_RIDE;
            return Match(state, ride, PAD_MASK);
        }

        private bool HitCrash(XBtns state)
        {
            if (rockBand1.Checked) return false;

            if (pS3GH5Kit.Checked)
                return Match(state, XBtns.RightShoulder, PAD_MASK);

            if (pS3EKit.Checked)
                return Match(state, FLAGS_CRASH_PS3, PAD_MASK);

            return Match(state, FLAGS_CRASH, PAD_MASK);
        }

        private static bool HitKickPedal(XBtns state)
        {
            return Match(state, FLAGS_KICK_PEDAL, PAD_MASK);
        }

        private bool HitHiHatPedal(XBtns state)
        {
            if (rockBand1.Checked) return false;
            var pedal = pS3EKit.Checked ? FLAGS_HIHAT_PEDAL_PS3 : FLAGS_HIHAT_PEDAL;
            return Match(state, pedal, PAD_MASK);
        }

        private bool HitFlam(XBtns state)
        {
            // Flam = simultaneous snare + Y-tom exact combos
            XBtns snare = FLAGS_SNARE, tomY = FLAGS_TOM_Y;
            if (pS3EKit.Checked) { snare = FLAGS_SNARE_PS3; tomY = FLAGS_TOM_Y_PS3; }
            else if (rockBand1.Checked) { snare = XBtns.B; tomY = XBtns.Y; }

            return Match(state, snare | tomY, PAD_MASK);
        }
        ...
        private void updater_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            ...
            using (var client = new WebClient())
            {
                try
                {
                    client.DownloadFile("https://nemosnautilus.com/drums/update.txt", path);
                }
                catch (Exception)
                {
                }
        ...
        private long ProcessMogg()
        {
            try
            {
                var stream = Bass.BASS_StreamCreateFile(nautilus.GetOggStreamIntPtr(false), 0L, nautilus.PlayingSongOggData.Length,
        ...
        private void StartPlayback()
        {
            ...
            BassStream = Bass.BASS_StreamCreateFile(nautilus.GetOggStreamIntPtr(false), 0L, nautilus.PlayingSongOggData.Length,
                BASSFlag.BASS_STREAM_DECODE | BASSFlag.BASS_SAMPLE_FLOAT);
        ...
        private void StopEverything()
        {
            ...
            picTrack.Invalidate();
            cboCharts.Invoke(new MethodInvoker(() => cboCharts.Items.Clear()));
            cboCharts.Invoke(new MethodInvoker(() => cboCharts.Text = ""));
        ...
        private void DrawNotes(Graphics graphics, bool doKicks)
        {
            ...
            for (var z = 0; z < track.ChartedNotes.Count(); z++)
            {
                ...
                if (styleRockBand.Checked && note_width > img.Width*0.98)
                {
                    note_width = img.Width * 0.98;
                }
        ...
        {
            Process.Start("https://rhythmgamingworld.com/");
        }
